...
class F1Optimizer():
    ...
    def maximize_expectation(P, pNone=None):
        """Given P, a sequence of product probabilities in descending order, return
          (k, predNone, max_f1)
        Where k is the index of the probability to use as threshold (non-inclusive)
        """
...
def print_best_prediction(P, pNone=None):
    ...
    best_k, predNone, max_f1 = F1Optimizer.maximize_expectation(P, pNone)
    best_prediction = ['None'] if predNone else []
    best_prediction += (L[:best_k])
    f1_max = max_f1
